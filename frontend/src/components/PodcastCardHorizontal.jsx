...
const PodcastCardHorizontal = ({ podcast, user, link }) => {
  ...
  return (
    <div className="relative">
      <Link
        to={`/${link}${podcast.id}`}
        className="flex items-center bg-[rgba(0,0,0,0.05)] rounded-lg p-3 hover:bg-[rgba(0,0,0,0.1)] transition-colors no-underline text-inherit"
      >
        ...
        <div className="flex-1 pl-4 flex flex-col justify-between h-full">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-500">
            <span className="truncate">{podcast.genre}</span>
            <span className="before:content-['•'] before:mx-2">
              {formatTimeAgo(podcast.timeAgo)}
            </span>
          </div>
        </div>
        {/* Live indicator badge on thumbnail */}
        {isLive && (
